...
import { clearSavedAssessment, loadSavedInput, loadSavedStep, saveInput, saveStep } from "@/lib/session-persistence"
...
const STEPS: AssessmentStep[] = [
  ...
  { id: "review", label: "Review", help: "Verify and generate recommendation" },
]
...
const STEP_META: Record<string, { title: string; description: string }> = {
  ...
  review: { title: "Review assessment", description: "Check the recorded information before generating the screening recommendation." },
}
...
export function AssessmentWizard() {
  ...
  const { input, updateSection, loadInput, submitting, error, setError, reset } = useAssessmentStore()
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <AssessmentShell
        ...
        utilityRail={utility}
        mobileActions={currentStep.id === "review" ? null : actions(true)}
      >
        <div className="flex min-w-0 flex-col gap-4">
          ...
          {currentStep.id !== "review" && error && <div role="alert" className="rounded-lg border border-urgency-referral-border bg-urgency-referral-soft p-4 text-sm font-medium leading-relaxed text-urgency-referral">{error}</div>}
          {currentStep.id !== "review" && <div className="hidden md:block">{actions()}</div>}
