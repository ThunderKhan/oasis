  ...
  if (!result) {
    return (
      <div className="mx-auto flex max-w-md flex-col items-center gap-4 rounded-card border border-border bg-surface px-6 py-14 text-center">
        ...
        <Link href="/assessment" className="inline-flex min-h-11 items-center justify-center rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary-hover focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">Start assessment</Link>
      </div>
    )
  }
    ...
    <>
      <div className="results-screen flex flex-col gap-6 pb-20 print:hidden md:pb-0">
        ...
        >
          <div className="flex flex-col gap-5 p-5 md:p-7">
            <div className="flex flex-col gap-4 md:flex-row-reverse md:items-start md:justify-between">
              <PriorityBadge priority={result.overall_priority} size="lg" className="self-start" />
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href="/assessment" onClick={onNewAssessment} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg px-4 text-sm font-medium text-muted transition-colors hover:bg-primary-soft hover:text-primary"><RotateCcw aria-hidden="true" className="size-4" />Start New Assessment</Link>
          <Link href="/referrals" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg px-4 text-sm font-medium text-muted transition-colors hover:bg-primary-soft hover:text-primary"><History aria-hidden="true" className="size-4" />View Assessment History</Link>
      ...
      <div
        className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-surface px-3 pt-3 print:hidden md:hidden"
