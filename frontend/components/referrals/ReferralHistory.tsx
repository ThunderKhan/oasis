    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-3 rounded-card border border-border bg-surface p-4 md:flex-row md:items-center" aria-label="Assessment filters">
        <label className="relative flex-1">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search assessment ID or patient code" className="h-10 w-full rounded-lg border border-border-strong bg-background pl-9 pr-3 text-sm outline-none focus:border-primary" />
        </label>
        <label>
          <span className="sr-only">Filter by priority</span>
          <select value={priority} onChange={(event) => setPriority(event.target.value as "all" | Priority)} className="h-10 w-full rounded-lg border border-border-strong bg-background px-3 text-sm md:w-52">
        ...
        <label className="md:hidden">
          ...
          <select value={sortKey} onChange={(event) => setSortKey(event.target.value as SortKey)} className="h-10 w-full rounded-lg border border-border-strong bg-background px-3 text-sm">
            ...
            <div className="max-h-[36rem] overflow-auto">
              <table className="w-full border-collapse text-left text-sm">
                <thead className="sticky top-0 bg-background text-xs uppercase tracking-wide text-muted">
                  <tr>{[["assessment_id", "Assessment ID"], ["patient_code", "Patient code"], ["overall_priority", "Overall priority"], ["created_at", "Date and time"]].map(([key, label]) => <th key={key} className="border-b border-border px-4 py-3"><button onClick={() => setSort(key as SortKey)} className="inline-flex items-center gap-1 font-semibold">{label}<ArrowDownUp aria-hidden="true" className="size-3" /></button></th>)}<th className="border-b border-border px-4 py-3 font-semibold">Status</th><th className="border-b border-border px-4 py-3 font-semibold">Action</th></tr>
                </thead>
...
function DisabledView() { return <button disabled aria-describedby="view-disabled-explanation" title="Detailed assessment retrieval will be enabled when the backend detail endpoint is added." className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted opacity-60">View</button> }
function HistoryRow({ record }: { record: AssessmentSummary }) { return <tr><td className="px-4 py-3 font-medium">{record.assessment_id}</td><td className="px-4 py-3">{record.patient_code}</td><td className="px-4 py-3"><PriorityBadge priority={record.overall_priority} size="sm" /></td><td className="px-4 py-3 text-muted">{formatDate(record.created_at)}</td><td className="px-4 py-3"><span className="rounded-full bg-urgency-routine-soft px-2 py-1 text-xs text-urgency-routine">Not tracked yet</span></td><td className="px-4 py-3"><DisabledView /></td></tr> }
function HistoryCard({ record }: { record: AssessmentSummary }) { return <Card><CardHeader className="p-4 pb-0"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-sm">{record.assessment_id}</CardTitle><p className="mt-1 text-xs text-muted">Patient {record.patient_code}</p></div><PriorityBadge priority={record.overall_priority} size="sm" /></div></CardHeader><CardContent className="flex items-end justify-between gap-3 p-4"><div className="text-xs text-muted"><p>{formatDate(record.created_at)}</p><p className="mt-1">Status: Not tracked yet</p></div><DisabledView /></CardContent></Card> }
...
function HistorySkeleton() { return <div className="flex flex-col gap-6" aria-label="Loading assessment history"><div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-24 animate-pulse rounded-card border border-border bg-surface" />)}</div><div className="h-16 animate-pulse rounded-card border border-border bg-surface" /><div className="h-80 animate-pulse rounded-card border border-border bg-surface" /></div> }
